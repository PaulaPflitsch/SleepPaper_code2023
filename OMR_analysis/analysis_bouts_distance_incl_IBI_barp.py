...
def boutInfo(raw_data, stimulus, num_bins):
    ...
    bdist = np.sqrt(pos_x**2 + pos_y**2)    # euclidian distance
...
def plotHistogram(experiment, num_bins, prob=False):
    ...
    '''
    #Paula: total distance covered
    distance_1 = sum(data_bdist_1)
    distance_2 = sum(data_bdist_2)
    mean_distance_1 = distance_1.mean()
    print(distance_1)
    mean_distance_2 = distance_2.mean()
    print("length mean_distance_1 1",mean_distance_1)

    x_range = range(4)
    f, ax = plt.subplots()

    sem_1 = sem(distance_1, axis=0, nan_policy='omit')
    sem_2 = sem(distance_2, axis=0, nan_policy='omit')

    ax.errorbar(x_range, mean_distance_1, yerr=sem_1, label='control', marker='o', markersize=2.0, color = 'black')
    ax.errorbar(np.array(range(mean_distance_2.shape[0])), np.sum(mean_distance_2 * mult, axis=1), yerr=sem_2, marker='o',
                markersize=2.0,
                label='continuous light', color='red')

    for i in x_range:
        x_1 = [i] * distance_1.shape[0]
        print("x1", x_1)
        print(distance_1[:, i])
        x_2 = [i] * distance_2.shape[0]

        dots = ax.scatter(x_1, distance_1[:, i], color='grey', alpha=0.2)
        jitter_dots(dots)
        dots = ax.scatter(x_2, distance_2[:, i], color='red', alpha=0.2)
        jitter_dots(dots)

    ax.set_xlabel(f'stimulus')
    ax.set_ylabel(f'Total distance covered')
    ax.set_title('Total distance covered')
    ax.legend()
    ax.grid(False)
    sns.despine(top=True, right=True)

    f.savefig(save_dir / f'fig_total_distance_covered.pdf')
    plt.close(f)

    # print mean, SEM, number of animals
    print("Mean control:", mean_distance_1[:4])
    print("SEM control", sem_1)
    print("Mean SD:", mean_distance_2[:4])
    print("SEM SD", sem_2)
    '''

    # bout distance histogram
    if stimuli % 2 == 0:
    ...
    for stimulus in range(half):
        ...
        ax1.plot(np.linspace(0,10,num_bins), data_ib_1[stimulus], label='control', color = 'grey')
        ax1.plot(np.linspace(0,10,num_bins), data_ib_2[stimulus], label='6h light', color = 'blue' )

        ax2.plot(np.linspace(0,0.15,num_bins), data_bdist_1[stimulus], label='control', color = 'grey')
        ax2.plot(np.linspace(0,0.15,num_bins), data_bdist_2[stimulus], label='6h light', color = 'blue' )
    ...
    if not prob:
        ...
        for i in x_range:
            ...
            jitter_dots(dots)
            dots = ax.scatter(x_2, raw_ibi_2[:, i], color='blue', alpha=0.2)
            jitter_dots(dots)
    ...
    # interbout interval as barplot for morning vs. afternoon

        f, ax = plt.subplots()

        #ax.bar([e + 1. for e in list(x_range)], freq_1, yerr=sem_1, label='control', alpha=0.5, width=0.4, color='grey')
        #ax.bar([e + 1.4 for e in list(x_range)], freq_2, yerr=sem_2, alpha=0.7, label='light day-1', width=0.4, color='red')
        ax.bar([e + 1. for e in list(x_range)], np.sum(data_ib_1 * mult, axis=1), yerr=sem_1,
                    label='control', color='grey',alpha=0.7,width=0.5)
        ax.bar([e + 1.5 for e in list(x_range)], np.sum(data_ib_2 * mult, axis=1), yerr=sem_2,
               label='6h light', color='blue', alpha=0.7,width=0.5)

        for i in x_range:
            x_1 = [i+1] * raw_ibi_1.shape[0]
            print("x1", x_1)
            print(raw_ibi_1[:, i])
            x_2 = [i+1.5] * raw_ibi_2.shape[0]
        ...
        f.savefig(save_dir / f'fig_avg_ib_interval_barplot.pdf')
        plt.close(f)
