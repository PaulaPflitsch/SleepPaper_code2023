...
def plotHistogram(experiment, prob=False):
    ...
    # plot cumulative sum plot_Paula

    # take data
    data_1 = raw_1*1000
    data_2 = raw_2*1000
    # evaluate the histogram
    values_1, base_1 = np.histogram(data_1, bins=50)
    values_2, base_2 = np.histogram(data_2, bins=50)
    # evaluate the cumulative
    cumulative_1 = np.cumsum(values_1)
    cumulative_2 = np.cumsum(values_2)
    # plot the cumulative function
    f, ax = plt.subplots()
    ax.plot(base_1[:-1], cumulative_1, c='black', label="control")
    ax.plot(base_2[:-1], cumulative_2, c="orange", label="0,1µM Melatonin")
    # plot the survival function
    #ax.plot(base_1[:-1], len(data_1) - cumulative_1, c='grey')
    #ax.plot(base_2[:-1], len(data_2) - cumulative_2, c='yellow')

    ax.set_xlabel('Time to first bout [ms]')
    ax.set_ylabel('Cumulative sum')
    ax.set_title('Cumulative sum of time to first bout')
    # ax.set_ylim(0,1000)
    ax.legend()
    ax.grid(False)
    ...
    f.savefig(save_dir / f'fig_first_cumsum.pdf')

    plt.close(f)

    # print mean and SEMs
    print('Mean control',mean_1 )
    print('SEM control', mean_sem_1)
    print('Mean melatonin', mean_2)
    print('SEM melatonin', mean_sem_2)
